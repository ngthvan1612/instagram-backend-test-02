...
public interface OrderRepository extends JpaRepository<Order, Integer>, ExtendOrderRepository {
  @Override
  @Query("SELECT CASE WHEN COUNT(u) > 0 THEN TRUE ELSE FALSE END FROM Order u WHERE u.id = :integer AND u.deletedAt is null")
  boolean existsById(@Param("integer") Integer integer);

  @Override
  @Query("SELECT u FROM Order u WHERE u.id = :integer AND u.deletedAt is null")
  Optional<Order> findById(@Param("integer") Integer integer);

  @Override
  @Query("SELECT u FROM Order u WHERE u.deletedAt is null")
  List<Order> findAll();
